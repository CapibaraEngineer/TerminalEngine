...
public:
    ...
    void updater(layerList &gameLayerList)
    {
        if (!isGameRunning)
        {
            ...
            {
                delete gameObjects.at(i); // Deleta os objetos do jogo, quando o jogo n�o est� rodando
            }
            return;
        }
        if (gameRenderer.render(gameLayerList, gameObjects) != 0)
        { // render error
            isGameRunning = false;
        }
    }

    // Verifica se o jogo est� em execu��o
    ...
    // Adiciona um novo objeto � lista de objetos do jogo
...
private:
    ...
    bool isGameRunning;                    // Indica se o jogo est� em execu��o
};
